...
class MissionViewSet(viewsets.ModelViewSet):
    ...
    def destroy(self, request, *args, **kwargs):
        ...
        if mission.cat is not None:
            return Response(
                {'error': 'Cannot delete mission, it`s assigned to a cat'},
                status=status.HTTP_400_BAD_REQUEST
            )
...
class TargetViewSet(viewsets.ModelViewSet):
    ...
    def update(self, request, *args, **kwargs):
        ...
        if target.is_complete:
            return Response({
                'error': 'Cannot update notes on a completed target'},
                status=status.HTTP_404_NOT_FOUND
            )
    ...
    def mark_complete(self, request, *args, **kwargs):
        ...
        if target.is_complete:
            return Response({
                'error': 'Target is already marked as complete.'},
                status=status.HTTP_400_BAD_REQUEST
            )
        ...
        return Response(
            {'status': 'Target marked as complete.'},
